...
public class BlocksCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("blocks")) {
            if (args.length > 1) {
                if (args[0].equalsIgnoreCase("set")) {
                    if (args.length == 3) {
                        ...
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', this.instance.config.getString("messages.prefix") +
            ...
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', this.instance.config.getString("messages.prefix") +
